...
| `00-backup.sh` | Salva `dconf` completo, configs (`xfce4`, `kdeglobals`, `.zshrc`…), o perfil de atalhos de teclado, lista de pacotes e tema de Plymouth em `~/.local/state/kali-look-backup/<data>/` | não | — (é o próprio ponto de retorno) |
...
| `11-wallpapers-kali.sh` | Os nove conjuntos de wallpaper oficiais do Kali, não só o do ano: `listar` (tamanho e estado, lidos do pool na hora), `baixar`, `instalar` (copia para o `$HOME`), `galeria [--html]`, `escolher` e `aplicar` (Xfce, Plasma ou GNOME) | não | `kali-look.sh remover assets --usuario` |
...
| `23-atalhos-teclado.sh` | Atalhos de teclado entre ambientes: `exportar` salva o conjunto efetivo (lido com `gsettings`, padrões do GNOME incluídos, que o `dconf dump` não traz), `status` compara com o do Xfce, `migrar` traduz para o canal `xfconf` do `xfwm4` (`--modo mesclar`/`exclusivo`), `mapa` mostra a tabela e o que não tem par | não | `kali-look.sh atalhos reverter` (restaura o canal do backup) |
