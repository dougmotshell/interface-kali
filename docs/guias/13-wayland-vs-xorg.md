...
| Atalhos personalizados | MUDA, mas dá para migrar | atalhos do GNOME não migram sozinhos | os **padrões** do GNOME também não aparecem no `dconf dump`, e são eles que você usa no dia a dia. `scripts/kali-look.sh atalhos status` compara e `… atalhos migrar` traduz o conjunto efetivo para o `xfwm4` — guia 04 §4.7 |
